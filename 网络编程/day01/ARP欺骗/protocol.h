...
{
	u_char eh_dst[6];		// 接收方MAC地址
	u_char eh_src[6];		// 发送方MAC地址
	u_short eh_type;		// 上层协议类型 
}EH_HEADR, *P_EH_HEADR;
//arp应答/请求(28字节)
...
typedef struct arp_hdr
{
	...
	u_char arp_hln;		     // 硬件（MAC）地址长度 
	u_char arp_pln;		     // 协议（IP ）地址长度 
	u_short arp_op;		     // 包类型：请求、应答
	u_char arp_sha[6];	     // 发送发硬件地址 （应答时，此处可欺骗）
	u_long arp_spa;		     // 发送方协议地址 （应答时，此处可欺骗）
	u_char arp_tha[6];	     // 接收方硬件地址 （请求时，此处无用）
	u_long arp_tpa;		     // 接收方协议地址 
