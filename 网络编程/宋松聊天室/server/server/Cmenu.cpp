...
Cmenu::Cmenu()
{
	...
	charaa.push_back("①  在线列表");//3
	charaa.push_back("②  实时群聊");//4
	charaa.push_back("③  远程控制");//5
	charaa.push_back("④  SQL查询");//6
	charaa.push_back("  ");//7
...
*/
bool Cmenu::main_menu(_In_ LPVOID lpParameter){
	...
	case 2:{
		//在线列表	
		MESSAGE aa;
		string showstring = Csocket::Allsuser();
		printf("%s", showstring.c_str());
		system("pause");
	}break;
	case 3:{
		//实时群聊
	...
	m_menu_y = 11;//重置光标位置
	m_min_y = 11;
	WriteChar(21, 1, " ", F_H_CYAN | F_H_CYAN);
	menu_choose();
...
void Cmenu::main_draw(){
	...
	WriteChar(26, 2, " ", F_H_CYAN | F_H_CYAN);
}
/*
