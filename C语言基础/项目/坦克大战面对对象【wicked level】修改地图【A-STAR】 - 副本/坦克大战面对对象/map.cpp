	...
	for (int i = 0; i < 256; i++)
	{
		...
		else
		{
			...
			case 0:
			{
				...
				if (!cheak_ball_bump(typea, b_position, (b_y-2), b_x, 1, i))
				{		 
			...
			case 2:
			{
				...
				if (!cheak_ball_bump(typea, b_position, (b_y), b_x - 2, 1, i))
				{
					all_ball[i].isBullet = 0;
				}
			...
			for (int k = 0; k < check_Rebottank; k++)
			{
				...
				for (int i = 0, lie = reboottank[k].m_y - 1; i < 3; lie++, i++){
					for (int j = 0, hang = reboottank[k].m_x - 1; j < 3; hang++, j++){
						if (lie == y && hang == x){
							if (reboottank[k].t_hp > 0 )//判断HP是否大于0
							{
								reboottank[k].t_hp--;
								return;
							}
...
功能：所有坦克 判断所有坦克的速度
*/
bool Calltank::Canmove(){
	finish = clock();//截至到当前 是否可以动作
	double  duration;
	duration = finish - start; /// CLOCKS_PER_SEC;
	if ((duration / 5) >= t_speed)//判断允许是否移动
	{
		start = clock(); //移动后 重新进行计数 
		return true;
	}
	else
	{
...
void CenemyTank::production_enemy(){
	...
	//	if (i == 1){}
			...
			if (reboottank[i].Wayfind == false && reboottank[i].realfind == true){
				...
				reboottank[i].sTart.nX = reboottank[i].m_x;
				reboottank[i].sTart.nY = reboottank[i].m_y;
				reboottank[i].sTart.nP = reboottank[i].t_position;
				reboottank[i].eNd.nX = mytanka[0].m_x;
				reboottank[i].eNd.nY = mytanka[0].m_y;
				reboottank[i].eNd.nP = mytanka[0].t_position;
				reboottank[i].Wayfind = 1;
			}
		
		if (reboottank[i].Wayfind == false){
			aa = rand() % 6; //不是自动寻路就用随机的
			reboottank[i].move_enemy(reboottank[i], aa);
		}
		else
		{
...
void CenemyTank::Wayfind_enemy(CenemyTank &aa,int tankid){
	...
	move_ok = move_enemy(reboottank[tankid], reboottank[tankid].m_Path[temp].nP);
	if (move_ok){
	//移动完删除最后面的位置
