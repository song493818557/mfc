...
void Castar::Getpath(int now_map[40][40]){
	...
	while (m_Openlist.size() != 0) //循环到没有可到达的方法为止
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			//cheak_bump(minArray[i].stcpos.nP, minArray[i].stcpos.nY, minArray[i].stcpos.nX, 9)
			//5、这四个点中  一下情况属于无效点
			
			/*5.2  假如是障碍物 *****************/
			//如果父节点和当前节点的方向不一样
			
			//WriteChar(minArray[i].stcpos.nY, minArray[i].stcpos.nX, "◇", 0 | F_H_RED);
			/*bool check = cheak_bump(minArray[i].stcpos.nP, minArray[i].stcpos.nY, minArray[i].stcpos.nX, 9);
			if (check == false) checkstate = false;*/
			
			//
			 if (now_map[minArray[i].stcpos.nY][minArray[i].stcpos.nX] == 3||
				now_map[minArray[i].stcpos.nY][minArray[i].stcpos.nX] == 4||
				now_map[minArray[i].stcpos.nY][minArray[i].stcpos.nX] == 5||
				now_map[minArray[i].stcpos.nY][minArray[i].stcpos.nX] == 6)
			{

				checkstate = false;
			}  
			int y = minArray[i].stcpos.nY;
			int x = minArray[i].stcpos.nX;
			int position = minArray[i].stcpos.nP;
			switch (position)
			{
			case 1:
			{
...
void Castar::PrintPath()
{
	...
	{
		WriteChar(m_Path[i].nY, m_Path[i].nX, "◇", 0 | F_H_RED);
	}
}
